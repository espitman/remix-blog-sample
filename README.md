...
یک وبلاگ مدرن و کامل ساخته شده با Remix، React، Prisma و PostgreSQL. این پروژه از shadcn/ui برای رابط کاربری استفاده می‌کند و دارای قابلیت‌های کامل CRUD برای مدیریت پست‌ها است. همچنین شامل یک سیستم نمایش اقامتگاه‌ها با استفاده از API های خارجی (Jabama) است.

## 🌟 ویژگی‌های جدید

- 🏨 **سیستم اقامتگاه‌ها**: نمایش اقامتگاه‌ها از API خارجی Jabama
- 🖼️ **Image Gallery Modal**: نمایش عکس‌ها در modal با navigation
- ⌨️ **Keyboard Navigation**: حرکت بین عکس‌ها با کلیدهای فلش کیبورد
- 🌐 **RTL Support**: پشتیبانی کامل از راست‌چین (Persian/Arabic)
- 🔤 **Vazirmatn Font**: استفاده از فونت فارسی Vazirmatn
- 📊 **Review Summary**: خلاصه نظرات مهمانان با استفاده از AI
...
- **[Vazirmatn Font](https://github.com/rastikerdar/vazirmatn)** - فونت فارسی برای RTL
...
│   │   └── accommodation-carousel.tsx  # کامپوننت Carousel اقامتگاه‌ها
...
│   │   ├── accommodations/      # منطق و تایپ‌های اقامتگاه
│   │   │   ├── accommodation.service.ts  # توابع API اقامتگاه‌ها
│   │   │   ├── accommodation.types.ts    # Type definitions
│   │   │   └── index.ts         # Export مرکزی
│   │   └── utils/                # توابع کمکی
│   │       ├── cn.ts            # تابع merge کلاس‌ها
│   │       └── index.ts         # Export مرکزی
│   ├── routes/                  # Route handlers (Remix)
│   │   ├── _index.tsx           # صفحه اصلی (لیست پست‌ها + Carousel اقامتگاه‌ها)
│   │   ├── posts.$slug.tsx      # صفحه نمایش پست
│   │   ├── accommodations.$code.tsx  # صفحه جزئیات اقامتگاه
...
- **ویژگی‌ها:**
  ...
  - تنظیم RTL (`dir="rtl"`) و زبان فارسی (`lang="fa"`)
  - Embed فونت Vazirmatn از Google Fonts
...
- **مسیر:** `/`
- **عملکرد:** نمایش لیست تمام پست‌ها + Carousel اقامتگاه‌ها
- **ویژگی‌ها:**
  - Hero section با گرادیان
  - Carousel اقامتگاه‌های ویژه (از API Jabama)
...
#### `app/routes/accommodations.$code.tsx`
- **مسیر:** `/accommodations/:code`
- **عملکرد:** نمایش جزئیات کامل یک اقامتگاه
- **ویژگی‌ها:**
  - Image Gallery با modal و navigation
  - Keyboard navigation (Arrow keys + Escape)
  - نمایش اطلاعات کامل اقامتگاه (قیمت، ظرفیت، امکانات)
  - خلاصه نظرات مهمانان (از AI API)
  - قوانین لغو رزرو
  - مکان‌های نزدیک
  - اطلاعات میزبان
  - طراحی RTL و فارسی
  - Sticky sidebar با اطلاعات رزرو
...
#### `app/lib/accommodations/` - منطق و تایپ‌های اقامتگاه
- **`accommodation.service.ts`**: شامل توابع API:
  - `getAccommodations(params)` - دریافت لیست اقامتگاه‌ها از Jabama API
  - `getAccommodationDetail(code)` - دریافت جزئیات یک اقامتگاه
  - `getAccommodationReviewSummary(code)` - دریافت خلاصه نظرات از AI API

- **`accommodation.types.ts`**: Type definitions:
  - `Accommodation` - نوع اقامتگاه (از search API)
  - `AccommodationDetail` - نوع جزئیات اقامتگاه
  - `ReviewSummary` - نوع خلاصه نظرات
  - و سایر types مرتبط
...
- **AccommodationCarousel** (`app/components/accommodation-carousel.tsx`) - Carousel اقامتگاه‌ها
  - نمایش افقی اقامتگاه‌ها
  - دکمه‌های navigation (قبلی/بعدی)
  - نمایش اطلاعات: قیمت، امتیاز، موقعیت، ظرفیت
  - لینک به صفحه جزئیات
  - طراحی RTL و فارسی

### 5. Image Gallery Modal

در صفحه جزئیات اقامتگاه (`/accommodations/:code`):
- **کلیک روی عکس**: باز شدن modal با عکس بزرگ
- **Navigation Buttons**: دکمه‌های قبلی/بعدی در راست و چپ
- **Keyboard Navigation**:
  - `ArrowRight (→)`: رفتن به عکس قبلی
  - `ArrowLeft (←)`: رفتن به عکس بعدی
  - `Escape`: بستن modal
- **Image Counter**: نمایش شماره عکس فعلی (مثلاً "1 / 20")
- **Close Button**: دکمه X برای بستن modal
- **Full Screen**: نمایش عکس در اندازه کامل با پس‌زمینه تیره
...
- 🕐 نمایش تاریخ و ساعت ساخت پست
- 🌐 پشتیبانی کامل RTL (راست‌چین)
- 🔤 فونت فارسی Vazirmatn
- 🏨 Carousel اقامتگاه‌ها
- 🖼️ Image Gallery Modal با navigation
- ⌨️ Keyboard navigation (Arrow keys + Escape)
- 📊 خلاصه نظرات مهمانان با AI
...
| Route | Description |
|-------|-------------|
| `/` | صفحه اصلی - لیست تمام پست‌ها + Carousel اقامتگاه‌ها |
| `/posts/:slug` | نمایش یک پست کامل |
| `/accommodations/:code` | نمایش جزئیات یک اقامتگاه |
...
- **RTL Support**: پشتیبانی کامل از راست‌چین با `dir="rtl"`
- **Vazirmatn Font**: فونت فارسی برای تمام متن‌ها
...
بعد از build، فایل‌های زیر را آپلود کنید:
- `build/` (کل فولدر)
- `node_modules/` (کل فولدر)
- `package.json`
- `package-lock.json`
- `.env` (با DATABASE_URL)
- `prisma/` (کل فولدر)
...
### نکات Deployment
- مطمئن شوید که `@remix-run/dev`, `prisma`, `vite` در `dependencies` هستند (نه `devDependencies`)
- Script `start` باید `remix-serve ./build/server/index.js` باشد
- Script `postinstall` باید `prisma generate` باشد
...
5. **Import Paths**: از مسیرهای جدید استفاده کنید:
   ...
   - `~/lib/accommodations/accommodation.service` برای توابع اقامتگاه
   - `~/lib/db/index.server` برای دیتابیس
   - `~/lib/utils/cn` برای utilities
6. **External APIs**: پروژه از API های خارجی استفاده می‌کند:
   - Jabama Search API برای لیست اقامتگاه‌ها
   - Jabama Detail API برای جزئیات اقامتگاه
   - Jabama Comments API برای خلاصه نظرات
7. **RTL & Persian**: تمام UI به فارسی و راست‌چین است
8. **Image Modal**: با کلیک روی عکس‌ها، modal باز می‌شود و می‌توانید با کلیدهای فلش حرکت کنید
